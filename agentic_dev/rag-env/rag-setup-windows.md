...
- **(Ditemukan & diperbaiki, 2026-09-07 -- hasil audit shortcoming post-fix, bukan bug dilaporkan pengguna)** `tools_called` yang dipakai `test_agent_cases.py` (diparsing dari `tool_calls.log`) SELALU melaporkan nama tool ASLI yang terpanggil (mis. `get_top_sellers`) walau `_maybe_correct_per_day_miss` sudah mengganti isinya jadi hasil `get_top_sellers_by_day` -- bentrok dengan `expected_tools` kasus seperti `PD04` (`["get_top_sellers_by_day"]`), sehingga perbandingan `actual_tools` vs `expected_tools` pada baris JSONL kasus yang dikoreksi otomatis SALAH menandai tool mismatch walau jawaban ke pengguna sudah benar (`score_autocorrect.py` sudah harus menghindari `tools_called` sama sekali karena ini, memakai pencocokan angka di teks jawaban sebagai gantinya). Diperbaiki: `_write_tool_log_line` sekarang menulis field tambahan `effective_tool=X` di baris log HANYA saat ada koreksi (format lama tanpa koreksi tidak berubah sama sekali, backward-compatible dgn baris log lama), dan `test_agent_cases.py::_new_tool_calls` membaca field ini utk `tools_called` (tool yg datanya sungguhan sampai ke pengguna) sambil tetap mengekspos nama asli terpisah lewat `raw_tools_called` (audit trail, tidak dipakai pencocokan `expected_tools`). Diverifikasi lewat `_verify_effective_tool_log_field.py` (4 kasus: giliran dikoreksi, giliran normal, baris log FORMAT LAMA tetap terparse, baris level-specialist tidak terpengaruh).
- **(Diselidiki, TIDAK ditemukan bukti kejadian nyata -- kanari log-only ditambahkan, evidence run 2026-09-07, lihat `arg_extraction_evidence_baseline.log`/`arg_extraction_evidence_after_narrowing.log`)** Diaudit sbg potensi celah: `_maybe_correct_per_day_miss` cuma mengoreksi TOOL SALAH terpanggil padahal argumen `start_date`/`end_date`-nya sendiri sudah benar berupa rentang -- kalau model gagal mengekstrak rentang tanggal itu SENDIRI dari pertanyaan (mis. cuma menangkap satu dari dua tanggal yang diminta), tidak ada jaring pengaman sama sekali. Ditambahkan `_log_possible_arg_extraction_miss()` (kanari LOG-ONLY, sengaja tidak mengoreksi apa pun -- parsing tanggal bahasa Indonesia bebas terlalu rawan salah utk jadi koreksi aktif tanpa bukti kejadian dulu, lihat `_maybe_correct_per_day_miss` di atas sbg preseden metodologi: kanari dulu, koreksi aktif cuma kalau ada bukti). Dijalankan langsung terhadap model sungguhan atas SELURUH 33 kasus yang menyebut rentang tanggal (grup RD*/PD*/DTB*/CPD*/LD05/MTD02) dengan kondisi awal (fire kalau start_date/end_date kosong salah satu ATAU sama) -> menyala 4x, TAPI keempatnya false positive terhadap desain yang MEMANG disengaja & sudah didokumentasikan di `test_agent_cases.py`: `DTB04`/`DTB05` (rentang satu-hari ditulis "tanggal X sampai X", start==end memang valid), `DTB07` (cuma end_date diisi -- sengaja default ke tanggal minimum tersedia), `CPD01` ("rentang" di pertanyaan itu rentang HARGA lewat `get_price_range`, bukan rentang tanggal). Nol dari 33 kasus menunjukkan kegagalan ekstraksi nyata. Kondisi dipersempit jadi HANYA sinyal paling jelas (start_date DAN end_date SAMA SEKALI kosong), run ulang 33 kasus yang sama -> nol kali menyala, tanpa kehilangan sensitivitas thd kegagalan total. Kanari dibiarkan berjalan permanen sbg observabilitas (pola sama seperti `EDW03` di bawah) -- **belum ada koreksi aktif** karena belum ada bukti kejadian nyata yg butuh dikoreksi.
