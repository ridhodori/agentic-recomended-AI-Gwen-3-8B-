...
- **(Ditemukan & diperbaiki, ditemukan lewat testing interaktif pengguna di luar 104-kasus)** Pertanyaan yang dibungkus sebagai permintaan kreatif/strategis (mis. "berikan aku 5 rekomendasi promo yang bisa dibuat berdasarkan 5 produk terlaris") membuat model menjawab lancar dengan nama produk, harga, dan persentase diskon spesifik TANPA memanggil tool apa pun -- dikonfirmasi lewat `tool_calls.log` (nol baris baru untuk giliran itu). Akar masalah: `SYSTEM_PROMPT` cuma mendaftar tool per JENIS pertanyaan (kategori/produk/harga/dst), tidak eksplisit menyebut bahwa framing "buatkan ide/rekomendasi/strategi" BUKAN alasan melewati tool kalau jawabannya tetap menyebut fakta penjualan konkret -- jadi model menafsirkannya sebagai pertanyaan opini yang boleh dijawab dari ingatan. Ini bukan gap kemampuan tool (semua tool yang dibutuhkan sudah ada), tapi model tidak dipanggil sama sekali. Diperbaiki: tambah paragraf baru di `SYSTEM_PROMPT` yang eksplisit menutup celah ini. Diverifikasi ulang dengan pertanyaan identik: model sekarang memanggil `get_top_sellers` (dan tool lain sesuai kebutuhan) sebelum menyusun ide promo, sehingga produk/harga/angka-terjual di jawaban berasal dari data nyata -- bagian yang tetap murni saran model sendiri (mis. persentase diskon promo) adalah aritmatika/ide di atas data nyata itu, bukan lagi produk atau angka penjualan yang dikarang.
